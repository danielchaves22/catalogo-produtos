...
export interface ExportacaoFabricantePayload {
  exportacaoId: number;
  superUserId: number;
}
...
export const exportacaoFabricanteJobHandler: AsyncJobHandler<ExportacaoFabricantePayload> = async ({
  job,
  payload,
  heartbeat,
}) => {
  if (!payload?.exportacaoId || !payload?.superUserId) {
    throw new Error('Payload da exportação de fabricantes inválido.');
  }

  const exportacao = await produtoExportacaoService.obterExportacaoPorId(payload.exportacaoId);

  if (!exportacao) {
    throw new Error('Registro de exportação de fabricantes não encontrado.');
  }
  ...
  await registerJobLog(job.id, AsyncJobStatus.PROCESSANDO, 'Preparando seleção de fabricantes.');
  await heartbeat();
  ...
  await registerJobLog(
    ...
    `Selecionando ${idsSelecionados.length} fabricante(s) para exportação.`
  );
  await heartbeat();

  const vinculos = await produtoExportacaoService.buscarFabricantesVinculados(idsSelecionados, payload.superUserId);
  const jsonDados = produtoExportacaoService.transformarFabricantesParaSiscomex(vinculos);
  ...
  await registerJobLog(
    job.id,
    AsyncJobStatus.PROCESSANDO,
    `Exportação de fabricantes gerada com ${jsonDados.length} item(ns). Arquivo disponível até ${expiraEm.toISOString()}.`
  );

  logger.info(`Exportação de fabricantes ${exportacao.id} concluída com ${jsonDados.length} itens.`);
};
